...
class ApiGrievanceService implements GrievanceService {
	...
	async withdraw(id: string): Promise<Result<{ ok: boolean }>> {
		const res = await fetch(`/api/grievances/${encodeURIComponent(id)}/withdraw`, {
			method: 'POST',
			credentials: 'include'
		});
		const json = await readJson(res);
		if (!res.ok) {
			return { ok: false, error: errorMessage(json, 'Could not withdraw grievance.') };
		}
		return { ok: true, data: { ok: true } };
	}

	async archive(id: string): Promise<Result<{ ok: boolean }>> {
		const res = await fetch(`/api/grievances/${encodeURIComponent(id)}/archive`, {
			method: 'POST',
			credentials: 'include'
		});
		const json = await readJson(res);
		if (!res.ok) {
			return { ok: false, error: errorMessage(json, 'Could not archive grievance.') };
		}
		return { ok: true, data: { ok: true } };
	}
}
