...
attachmentRoutes.get('/:id', (c) => {
	...
	// SEC-09: Use RFC 5987 encoded filename* to prevent header injection
	const encodedName = encodeURIComponent(row.original_filename).replaceAll("'", '%27').replaceAll('(', '%28').replaceAll(')', '%29');
	c.header(
		'Content-Disposition',
